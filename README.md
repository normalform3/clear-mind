...
Clear Mind 是一个本地优先的原生 macOS 应用，用来托管长期目标、近期事项和暂时无法处理的想法。它不包含账号、云同步、遥测，也不尝试替代每日 Todo。安心总览顶部显示当天日期和当前时间，下方的“时间表”呈现一天的安排。
...
长期目标默认显示横向规划图：整个目标周期适配页面宽度，每个推进项按实际起止日期占一行，可直观看到持续时间与并行关系。图中标出今天、当前推进项和月份分界线；“时间进度”只表示日期位置，不代表完成情况。悬停可查看今日或推进项的起止日期，点击推进条或里程碑可打开表单。
